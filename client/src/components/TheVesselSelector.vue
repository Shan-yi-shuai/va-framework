<template>
  ...
  const { vessels, selectedVesselIDs, vesselID2Name, vesselName2ID } = storeToRefs(store)
  ...
  const value = computed({
    get: () => selectedVesselIDs.value.map(id => vesselID2Name.value[id]),
    set: (val: CheckboxValueType[]) => {
      selectedVesselIDs.value = val.map(name => vesselName2ID.value[name])
    }
  })
